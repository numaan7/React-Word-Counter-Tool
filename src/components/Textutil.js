...
function Textutil(props) {
  ...
  return (
    <div >
      ...
      <span  className='m-3'>⏱️ Time to read: <b id='wordtime'>0</b> minutes</span>
      <div className="m-3">

        <textarea onChange={updateword} value={word} className={`form-control border border-0  ${(props.mode === 'dark') ? 'bg-dark text-light' : 'bg-light'} `} id="textarea" rows="8" placeholder="Type or paste your text here... 🖊️"></textarea>
      </div>
      <div className="px-3">
        <button onClick={() => {
        ...
        <button onClick={() => {
          setWords('')
          props.showAlert("🗑️ Text is cleared now!", "success");
        }} className="btn btn-danger" type="button">🗑️ Clear text</button>
      </div>
